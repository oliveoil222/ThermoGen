...
class Ui_TechDemoMainWindow(object):
    ...
    def ButtonClickedAsync(self):
        start_temp = currentTemperature
        start_pres = currentPressure
        desired_temp = self.tempValueSelector.value()
        desired_pres = self.pressureValueSelector.value()

        self.Worker = Worker(start_temp, start_pres, desired_temp, desired_pres,
                             sorensen_addr=None, daq_channel=None, pres_serial_port=None)
